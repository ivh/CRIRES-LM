...
# dependencies = ["pandas"]
# ///
"""Generate SOF files that combine all raw frames per template start."""
...
for tpl_start, group in df.groupby('tpl_start'):
    ...
    combdir = outdir / dirname
    combdir.mkdir(exist_ok=True)

    sof = combdir / 'nodd.sof'
...
print(f'{n_dirs} combined SOF directories created')

# summary by setting
by_setting = {}
for tpl_start, group in df.groupby('tpl_start'):
    setting = group.iloc[0]['ins_wlen_id']
    by_setting.setdefault(setting, 0)
    by_setting[setting] += 1

print('\nSequences per setting:')
